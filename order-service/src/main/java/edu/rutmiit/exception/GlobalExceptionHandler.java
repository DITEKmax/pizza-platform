...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(DublicateItemException.class)
    public ResponseEntity<ErrorResponse> handleDublicateConflict(DublicateItemException ex, HttpServletRequest req) {
        return ResponseEntity
                .status(HttpStatus.CONFLICT)
                .body(new ErrorResponse(
                        HttpStatus.CONFLICT.value(),
                        BASE_PROBLEM_URI + "dublicate-conflict",
                        "Конфликт с существующим объектом",
                        ex.getMessage(),
                        req.getRequestURI(),
                        Instant.now(),
                        null
    ...
    public ResponseEntity<ErrorResponse> handleOrderCancelConflict(OrderNotCancellableException ex, HttpServletRequest req) {
        return ResponseEntity
                .status(HttpStatus.CONFLICT)
                .body(new ErrorResponse(
                        HttpStatus.CONFLICT.value(),
                        BASE_PROBLEM_URI + "order-not-cancellable-conflict",
